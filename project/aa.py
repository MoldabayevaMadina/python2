...
def finish_booking_tables(window_show_tables):
...
def finish_booking_vip(window_show_vip):
...
def menu(window_show_vip):
    ...
    Button(window_menu, text = 'finish\nbooking', font = ("Arial", 10), background = "white", command = lambda: finish_booking_vip(window_menu)).place(x = 420, y = 200)
...
def show_vip(window_vip1, Enum_of_people, Edate_vip):
    ...
    if num_of_people == '' or date_vip == '':
        ...
        if int(y) < now_year:
            ...
            Button(window_show_vip, text = 'Next', font = ("Arial", 10), background = "white", command = lambda: menu(window_show_vip)).place(x = 430, y = 380, width = 50, height = 20)
            query0 = 'SELECT hall_number FROM vip_users WHERE vip_data = %s'
            value0 = (date_vip,)
        ...
        #    print(list_of_tables)
            for i in range(len(list_of_vips)):
                var3 = IntVar()
                var3.set(-1)
                Checkbutton(window_show_vip,  text= f'      {list_of_vips[i][0]}                                               {hall_seats_vip[i]}                                                {price_vip[i]}', variable=var3, onvalue=1, offvalue=0, bg = 'white').place(x = 30, y = 100 + 20 * i)
...
def show_tables(window_tables, combo, Edate):
    ...
    if date1 == '':
        ...
        if int(y) < now_year:
            ...
            window_show_tables.title('CHOOSE333')
            window_show_tables.geometry('500x550')
            ...
            query = ('SELECT table_number FROM users WHERE data = %s and meal = %s')
            ...
            for i in range(len(list_of_tables)):
                #Label(window_show_tables, text = '***', background = "white",  font = ('Arial', 12)).place(x = 30, y = 150 * k)
                #print(i)
                var2 = IntVar()
                var2.set(-1)
                #Checkbutton(window_show_tables,  text= f'      {list_of_tables[i][0]}                                                 {list_of_tables[i][1]}                                                {list_of_tables[i][2]}', variable=var2, onvalue=1, offvalue=0, bg = 'white').place(x = 30, y = 100 + 20 * i)
                Checkbutton(window_show_tables,  text= f'      {list_of_tables[i][0]}                                                       {num_seats_tables[i]}                                          {price_tables[i]}  ', variable=var2, onvalue=1, offvalue=0, bg = 'white').place(x = 30, y = 100 + 20 * i)

                #L = Label(window_show_tables, text = f'{list_of_tables[i][0]}                                          {list_of_tables[i][1]}                           {list_of_tables[i][2]}', background = "white",  font = ('Arial', 9))
                #L.place(x = 70, y = 100 + 20 * i)
            Label(window_show_tables, text = 'table_number                    number of seats                   price  ', background = "white", font = ('Arial', 10)).place(x = 40, y = 70)
            Button(window_show_tables, text = 'finish\nbooking', font = ("Arial", 10), background = "white", command = lambda: finish_booking_tables(window_show_tables)).place(x = 420, y = 500)
            ...
            Button(window_show_tables, text = 'Back', font = ("Arial", 10), background = "white", command = lambda: go_back(window_tables, window_show_tables)).place(x = 20, y = 510, width = 50, height = 20)
